#!/usr/bin/env bash
# guard-destructive-ops.sh — PreToolUse safety hook for platform-infrastructure
...
for pattern in "${DESTRUCTIVE_PATTERNS[@]}"; do
  if echo "${TOOL_INPUT}" | grep -qE "(^|[^a-zA-Z])${pattern}"; then
    echo "⚠  SAFETY HOOK: destructive pattern detected (${pattern}) — platform-infrastructure"
